...
const InputForm = ({ onStartGame, error }) => {
  ...
  const [attemptsCountInput, setAttemptsCountInput] = useState('');
  const navigate = useNavigate();

  const handleSubmit = (e) => {
    e.preventDefault();
    const carNames = carNamesInput.split(',').map(name => name.trim());
    onStartGame(carNames, attemptsCountInput);
  };
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                  id="attemptsCount"
                  type="number"
                  value={attemptsCountInput}
                  onChange={(e) => setAttemptsCountInput(e.target.value)}
                  placeholder="5"
                  min="1"
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
              />
          ...
          <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col items-center">
            ...
            <button
                className="mt-6 px-4 py-2 rounded bg-yellow-300 font-bold hover:bg-yellow-400 transition"
